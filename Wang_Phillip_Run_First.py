...
csvfile2=open('dog_info.csv', 'w')
dog_writer = csv.writer(csvfile2, delimiter=',')
...
def breed_page(): # Get background info for each dog breed
    ...
    holder=[]
    try:
        for i in Breeds_list[:50]:  #Convert spaces in Breed Name to hyphens inorder for url to work
            if ' ' in i:
                i=i.replace(' ','-')
            else:
                pass
...
for i in Breeds_list[:50]: #Driver function to get all Breed adoption listings and write them into dog_adoption csv
...
for item in holder: # Take info from each dog list and parse them into each respective category
    ...
    for items in item:
        ...
        if count==15:
            if track==0:
                dog_writer.writerow(columns)
                dog_writer.writerow(index)
                track+=1
            elif track>=1:
                dog_writer.writerow(index)
